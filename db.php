...
if (!function_exists('normalize_election_type')) {
    function normalize_election_type($value): string
    {
        $election = strtoupper(trim((string) $value));
        if (!in_array($election, ['SSG', 'FTP'], true)) {
            $election = 'SSG';
        }
        return $election;
    }
...
if (!function_exists('candidate_position_rank')) {
    function candidate_position_rank(string $electionType, string $position): int
    {
        ...
        $electionKey = normalize_election_type($electionType);
